...
fn main() {
    let clp = Command::new("Zcif bin")
        ...
        .arg(
            arg!(-r --runs <run> "Run the encoder/decoder <run> number of times")
                .required(false)
                .default_value("20"),
        )
        .get_matches();
    ...
    if clp.value_of("fse").is_some() {
        ...
        unsafe { _fse(clp.value_of("fse").unwrap(), runs) };
    } else if clp.value_of("huff").is_some() {
        println!("Opening file: {:?}", clp.value_of("huff").unwrap());
        unsafe { _huff(clp.value_of("huff").unwrap(), runs) };
    }
    // _fse();
...
unsafe fn _huff(file: &str, runs: usize) {
    ...
    for _ in 0..runs {
        ...
        huff_decompress(writer.get_ref(), &mut new_writer).unwrap();
        let stop = {
            #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
            {
                _rdtsc()
        ...
        eprint!(
            "{:.2?} cycles per byte ",
            (stop - start) as f64 / (new_writer.len()) as f64
        );
        eprint!(
            " {:.2} MB/s",
...
unsafe fn _fse(file: &str, runs: usize) {
    let writer = Vec::with_capacity(2);
    let mut writer = std::io::Cursor::new(writer);
    writer.set_position(0);
    ...
    for _ in 0..runs {
        ...
        fse_decompress(writer.get_ref(), &mut new_writer).unwrap();
        let stop = {
            #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
            {
                _rdtsc()
